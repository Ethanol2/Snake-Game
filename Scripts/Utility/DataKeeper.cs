using Godot;
using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Linq;

public class DataKeeper
{
	private static DataKeeper _instance;

	private string _scoreFilePath = "";

	private Data _data;

	public static DataKeeper Instance => _instance;

	public static bool Initialized => _instance;
	public static string LatestName
	{
		get => _instance ? _instance._data.LatestName : System.Environment.UserName;
		set { if (_instance) _instance._data.LatestName = value; }
	}
	public static int ActiveRuleSet
	{
		get => IsInitialized() ? _instance._data.ActiveRuleSet : 12000;
		set { if (IsInitialized()) _instance._data.ActiveRuleSet = value; }
	}

	public static void Init(string filePath)
	{
		if (_instance != null)
		{
			_instance.LogError("An instance already exists", true, 2);
		}
		_instance = new DataKeeper();
		_instance._Init(filePath);
	}
	private void _Init(string filePath)
	{
		_scoreFilePath = filePath;

		if (FileAccess.FileExists(filePath))
		{
			string fileContents;
			using (FileAccess file = FileAccess.Open(filePath, FileAccess.ModeFlags.Read))
			{
				if (file == null)
				{
					this.LogError($"There was an issue opening \"{filePath}\" ->\n" + FileAccess.GetOpenError());
					return;
				}
				fileContents = file.GetAsText();
				file.Close();
			}

			if (fileContents != string.Empty)
			{
				try
				{
					_data = System.Text.Json.JsonSerializer.Deserialize<Data>(fileContents);
					return;
				}
				catch (Exception e)
				{
					this.LogError("Something went wrong parsing the score file ->\n" + e);
				}
			}
		}

		_data = new Data();
		_data.LatestName = System.Environment.UserName;
	}
	public static List<int> GetScores(int ruleSet)
	{
		if (!IsInitialized())
			return null;

		Data.RuleSetScores scoresData = _instance._data[ruleSet];

		List<int> scores = new List<int>();
		foreach (string name in scoresData.Scores.Keys)
		{
			scores.AddRange(scoresData.Scores[name]);
		}
		return scores;
	}
	public static Dictionary<string, List<int>> GetScoresAndNames(int ruleSet)
	{
		if (IsInitialized())
			return _instance._data[ruleSet].Scores;
		return null;
	}
	public static void AddScore(int ruleSet, string name, int score)
	{
		if (IsInitialized())
		{
			_instance._data.LatestName = name;

			Data.RuleSetScores scores = _instance._data[ruleSet];
			scores.AddScore(name, score);
			_instance._data[ruleSet] = scores;
		}
	}
	public static void WriteToDisk()
	{
		if (IsInitialized())
			_instance._WriteToDisk();
	}
	private void _WriteToDisk()
	{
		string jsonText;
		try
		{
			jsonText = System.Text.Json.JsonSerializer.Serialize<Data>(
				_data,
				new System.Text.Json.JsonSerializerOptions() {WriteIndented=true});
		}
		catch (Exception e)
		{
			this.LogError("Something went wrong writing to disk ->\n" + e);
			return;
		}

		using (FileAccess file = FileAccess.Open(_scoreFilePath, FileAccess.ModeFlags.Write))
		{
			if (file == null)
			{
				this.LogError($"There was an issue opening \"{_scoreFilePath}\" ->\n" + FileAccess.GetOpenError());
				return;
			}

			file.StoreString(jsonText);
			file.Close();
		}
	}
	public static void ClearSaveFile()
	{
		if (IsInitialized())
			_instance._ClearSaveFile();
	}
	public void _ClearSaveFile()
	{
		_data = new Data();
		_WriteToDisk();
	}
	public static void TestDataWrite()
	{
		if (!IsInitialized()) return;

		_instance._data.Scores = new List<Data.RuleSetScores>()
		{
			new Data.RuleSetScores() {
				Scores = new Dictionary<string, List<int>>
			{
				{ "Bob", new List<int>() {10, 15} },
				{ "Jim", new List<int>() {5, 30} },
				{ "Jane", new List<int>() {6, 1}},
				{ "Craig", new List<int>() {1, 1}}
			},
				RuleSet = 12
			}
		};

		WriteToDisk();
	}

	private static bool IsInitialized()
	{
		if (_instance) return true;

		DebugLog.Log("ScoreKeeper", "Not initialized");
		return false;
	}

	public class Data
	{
		public string LatestName { get; set; } = "";
		public List<RuleSetScores> Scores { get; set; } = new List<RuleSetScores>();
		public int ActiveRuleSet { get; set; } = -1;

		public class RuleSetScores
		{
			public int RuleSet { get; set; } = -1;
			public Dictionary<string, List<int>> Scores { get; set; } = new Dictionary<string, List<int>>();

			public void AddScore(string name, int score)
			{
				if (Scores.ContainsKey(name))
					Scores[name].Add(score);
				else
					Scores.Add(name, new List<int>() { score });
			}
		}

		public RuleSetScores this[int rules]
		{
			get
			{
				if (rules < 0)
				{
					throw new Exception("Invalid rule set: " + rules);
				}

				foreach (RuleSetScores scores in Scores)
						if (scores.RuleSet == rules)
							return scores;
				return new RuleSetScores() {RuleSet=rules};
			}
			set
			{
				if (rules < 0)
				{
					throw new Exception("Invalid rule set: " + rules);
				}

				for (int i = 0; i < Scores.Count; i++)
				{
					if (Scores[i].RuleSet == rules)
					{
						Scores[i] = value;
						return;
					}
				}
				Scores.Add(value);
			}
		}
	}
	
	public static implicit operator bool(DataKeeper instance) => instance != null;
}
